...
(function($B) {
  ...
  $B.$syntax_err_line = function(exc, module, pos, lineNum, msg) {
    // map position to line number
    var pos2line = {}
    var lnum = 1
    var src = $B.$py_src[module] || $SRC_CODE;
    if (src === undefined) {
    ...
    if (lineNum === undefined) {
      ...
      lineNum = pos2line[pos]
    }
    exc.$line_info = lineNum + ',' + module

    var lines = src.split('\n')
    var line = lines[lineNum - 1] || ''
    var lpos = pos - line_pos[lineNum]
    var lpos2 = lpos // **
    var len = line.length
    lpos2 -= len - line.length // **
    line = line.replace(/^\s*/, '')
    lpos -= len - line.length
    exc.$linterRow = lineNum - 1
    exc.$linterCol = lpos2
    exc.$linterMessage = msg
    exc.args = _b_.tuple([
      $B.$getitem(exc.args, 0),
      module,
      lineNum,
      lpos,
      line
  ...
  $B.$SyntaxError = function(module, msg, pos, lineNum, root) {
    ...
    $B.$syntax_err_line(exc, module, pos, lineNum, msg)
    throw exc
  }

  $B.$IndentationError = function(module, msg, pos) {
    var exc = _b_.IndentationError(msg)
    $B.$syntax_err_line(exc, module, pos, undefined, msg)
    throw exc
  }
    ...
    switch (attr) {
      ...
      case 'tb_next':
        if (self.stack.length == 1) {
          ...
          return traceback(self.stack.slice(0, self.stack.length - 1))
        }
      default:
  ...
  $TracebackDict.__str__ = function(self) {
    return '<traceback object>'
  }

  function traceback(stack) {
    return {__class__: $TracebackDict, stack: stack}
  }
  ...
  $BaseExceptionDict.__init__ = function(self) {
    var args = arguments[1] === undefined
      ? []
      : [arguments[1]]
      ...
      for (var i = 0; i < self.$stack.length; i++) {
        var frame = self.$stack[i]
          ...
          var line_info = frame[1].$line_info.split(',')
            if ($B.$py_src[line_info[1]] === undefined) {
              continue
            }
            ...
            if (line)
              line = line.replace(/^[ ]+/g, '')
            if (line === undefined) {
              console.log('line undef...', line_info, $B.$py_src[line_info[1]])
            }
            info += '\n    ' + line
          }
          return info
          ...
          throw AttributeError(self.__class__.__name__ + "has no attribute '" + attr + "'")
        }
      }
        ...
        if (!js_exc.$py_error) {
            ...
            if (_frame && _frame[1].$line_info !== undefined) {
              ...
              if (module) {
                ...
                var lib_module = mod_name
                  var lineNum = parseInt(line_info[0])
                    ...
                    msg += "\n  module '" + lib_module + "' line " + lineNum
                    msg += '\n' + lines[lineNum - 1]
                    js_exc.msg = msg
          ...
          $B.is_exc = function(exc, exc_list) {
            ...
            for (var i = 0; i < exc_list.length; i++) {
              if (issubclass(exc_class, exc_list[i]))
                return true
            }
            return false
          }
          ...
          $make_exc([
            'EnvironmentError', 'IOError', 'VMSError', 'WindowsError'
          ], _b_.OSError)
